...
static void draw_separator(i3_output *output, uint32_t x, struct status_block *block, bool use_focus_colors) {
    ...
    if (config.separator_symbol == NULL) {
        ...
        draw_util_text(config.separator_symbol, &output->statusline_buffer, sep_fg, bar_bg,
                       separator_x, logical_px(ws_margin_px + ws_voff_px - ws_border_px), x - separator_x);
    }
}
    ...
    TAILQ_FOREACH (block, &statusline_head, blocks) {
        ...
        if (block->border || block->background || block->urgent) {
            ...
            draw_util_rectangle(&output->statusline_buffer, border_color,
                                x, logical_px(ws_border_px),
                                full_render_width,
                                bar_height - logical_px(2 * ws_border_px));

            /* Draw the background. */
            draw_util_rectangle(&output->statusline_buffer, bg_color,
                                x + has_border * logical_px(block->border_left),
                                logical_px(ws_border_px) + has_border * logical_px(block->border_top),
                                full_render_width - has_border * logical_px(block->border_right + block->border_left),
                                bar_height - has_border * logical_px(block->border_bottom + block->border_top) - logical_px(2 * ws_border_px));
        }
...
static int predict_button_width(int name_width) {
    return MAX(name_width + 2 * logical_px(ws_hoff_px) + 2 * logical_px(ws_border_px),
...
void init_xcb_late(char *fontname) {
    ...
    bar_height = font.height + 2 * (logical_px(ws_margin_px + ws_voff_px - ws_border_px));
...
static void draw_button(surface_t *surface, color_t fg_color, color_t bg_color, color_t border_color,
    ...
    int height = font.height + 2 * logical_px(ws_voff_px) - 2 * logical_px(ws_border_px);

    /* Draw the border of the button. */
    draw_util_rectangle(surface, border_color, x, logical_px(ws_margin_px), width, height);

    /* Draw the inside of the button. */
    draw_util_rectangle(surface, bg_color, x + logical_px(ws_border_px),
                        logical_px(ws_margin_px) + logical_px(ws_border_px),
                        width - 2 * logical_px(ws_border_px),
                        height - 2 * logical_px(ws_border_px));

    draw_util_text(text, surface, fg_color, bg_color, x + (width - text_width) / 2,
                   logical_px(ws_margin_px + ws_voff_px - ws_border_px), text_width);
}
